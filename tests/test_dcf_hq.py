...
from dcf_hq.markets import normalize_market, market_routes
from dcf_hq.platforms import platform_supported, platform_requires_media
from dcf_hq.approval import can_approve_content, enforce_approval_state, lifecycle_transition


def test_market_normalization_and_routes():
    assert normalize_market("egypt") == "EG"
    assert normalize_market("uae") == "AE"
    assert market_routes("sa")["default"] == "Amazon Saudi Arabia"


def test_platform_caps_and_media_requirements():
    assert platform_supported("X") is True
    assert platform_requires_media("TIKTOK") is True
    assert platform_requires_media("THREADS") is False


def test_approval_gate_and_lifecycle():
    ready = can_approve_content({"product_id": "p100", "market": "US", "cta": "See it", "affiliate_destination": "https://example.com"})
    assert ready.status == "READY"
    blocked = enforce_approval_state({"product_id": "p100", "market": "US", "cta": "See it", "affiliate_destination": "https://example.com"}, "PENDING")
    assert blocked.status == "BLOCKED"
    assert lifecycle_transition("QA", "READY_FOR_APPROVAL") is True
